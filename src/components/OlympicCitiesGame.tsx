...
interface OlympicCitiesGameProps {
  onFinishGame: (finalScore: number, gameId: string) => void;
  onBackToMenu: () => void;
  currentLang: "en" | "ru" | "uz";
}

// 2. Olympic Hosts Data List (10 historical games)
const OLYMPIC_HOSTS: OlympicHost[] = [
  {
    year: 1964,
    season: "Summer",
    city: "Tokyo",
    ...
    lat: 35.6762,
    lon: 139.6503,
    fact: "The first Olympic Games held in Asia, introducing the world to real-time international telecasts."
  },
  {
    year: 1992,
    season: "Summer",
    ...
    lat: 41.3851,
    lon: 2.1734,
    fact: "The legendary USA basketball 'Dream Team' dominated, and the Olympic cauldron was lit by an archer firing a flaming arrow."
  },
  {
    year: 2000,
    season: "Summer",
    ...
    lat: -33.8688,
    lon: 151.2093,
    fact: "Widely declared the 'best Olympic Games ever' by IOC President Samaranch, with Cathy Freeman lighting the cauldron."
  },
  {
    year: 2004,
    season: "Summer",
    ...
    lat: 37.9838,
    lon: 23.7275,
    fact: "The Olympics returned to their historic birthplace, and shot put events were held in the ancient stadium of Olympia."
  },
  {
    year: 2008,
    season: "Summer",
    ...
    lat: 39.9042,
    lon: 116.4074,
    fact: "Pioneering games where swimmer Michael Phelps won 8 gold medals and sprinter Usain Bolt set legendary world records."
  },
  {
    year: 2012,
    season: "Summer",
    ...
    lat: 51.5074,
    lon: -0.1278,
    fact: "London became the first city to host the modern Olympic Games three times (1908, 1948, and 2012)."
  },
  {
    year: 2016,
    season: "Summer",
    ...
    lat: -22.9068,
    lon: -43.1729,
    fact: "The first Olympic Games held in South America, welcoming the inspiring debut of the Refugee Olympic Team."
  },
  {
    year: 1960,
    season: "Summer",
    ...
    lat: 41.9028,
    lon: 12.4964,
    fact: "Cassius Clay (later Muhammad Ali) won light heavyweight gold, and Ethiopia's Abebe Bikila won the marathon running barefoot."
  },
  {
    year: 1972,
    season: "Summer",
    ...
    lat: 48.1351,
    lon: 11.5820,
    fact: "Famous for gymnast Olga Korbut winning three gold medals and swimmer Mark Spitz securing seven gold medals."
  },
  {
    year: 1996,
    season: "Summer",
    ...
    lat: 33.7490,
    lon: -84.3880,
    fact: "Marked the centennial of the modern Olympic Games. Muhammad Ali lit the cauldron during a touching opening ceremony."
  }
];
...
export default function OlympicCitiesGame({
  onFinishGame,
  onBackToMenu,
  currentLang = "en",
}: OlympicCitiesGameProps) {
  // Game states
  ...
  const currentHost = OLYMPIC_HOSTS[currentRoundIndex];
  ...
  const handleLockIn = () => {
    ...
    const distance = calculateDistanceKM(guessLat, guessLon, currentHost.lat, currentHost.lon);
    ...
    const actualMarker = L.marker([currentHost.lat, currentHost.lon], { icon: correctIcon })
      .addTo(mapRef.current)
      .bindTooltip(`<b>${currentHost.city}</b> (${currentHost.country})`, { permanent: true, direction: "top", offset: [0, -10] });

    actualMarkerRef.current = actualMarker;

    const connectorLine = L.polyline([guessCoords, [currentHost.lat, currentHost.lon]], {
      color: "#4b5563",
      weight: 2,
      dashArray: "6, 6",
    }).addTo(mapRef.current);
  ...
  const handleNextRound = () => {
    ...
    if (currentRoundIndex + 1 < OLYMPIC_HOSTS.length) {
  ...
  return (
    <div className="w-full flex flex-col gap-6" id="olympic_cities_wrapper">
      ...
      <div className="flex items-center justify-between border-b border-gray-200 pb-3" id="game_header_nav">
        ...
        <div className="text-right">
          ...
          <span className="text-xs font-black text-gray-900 uppercase">Olympic Cities</span>
        </div>
      </div>
      ...
      {!isGameOver ? (
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch" id="game_running_grid">
          ...
          <div className="lg:col-span-8 flex flex-col gap-3" id="game_maps_stage_side">
            ...
            <div className="bg-white border border-gray-200 rounded-[12px] p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm" id="round_stat_display">
              <div>
                <span className="text-[10px] font-mono font-bold text-gray-400 uppercase tracking-widest block">
                  ROUND {currentRoundIndex + 1} OF {OLYMPIC_HOSTS.length}
                </span>
                <h1 className="text-xl sm:text-2xl font-black text-gray-955 uppercase tracking-tight">
                  Where was the <span className="underline decoration-black decoration-2 underline-offset-2">{currentHost.year} {currentHost.season} Olympics</span>?
                </h1>
                <p className="text-xs text-gray-500 font-semibold mt-1">
                  Pin the historic host city of the global athletic festival
                </p>
              </div>
            ...
            <div className="relative flex-1 flex flex-col bg-white rounded-[12px] border border-gray-200 p-1.5 shadow-sm overflow-hidden min-h-[420px] lg:min-h-[500px]" id="olympic_map_holder">
              ...
              {!guessCoords && (
                <div className="absolute top-4 left-4 right-4 sm:left-auto z-40 bg-black/90 backdrop-blur-sm border border-gray-700 px-4 py-2.5 rounded-[8px] shadow-md text-xs font-semibold text-white pointer-events-none text-center sm:text-left select-none animate-pulse">
                  📍 Click on the map to pin this Olympic host city!
                </div>
              )}

              <div ref={mapContainerRef} className="w-full h-full min-h-[420px] lg:min-h-[520px] custom-leaflet-map" id="olympic_leaflet_stage" style={{ zIndex: 10 }} />
            </div>
          </div>
                ...
                {!guessCoords && (
                  <div className="text-center py-8 text-gray-400" id="op_waiting_guess">
                    ...
                    <p className="text-[11px] text-gray-500 leading-normal px-4 mt-1 font-semibold">
                      Click the map where you think the {currentHost.year} {currentHost.season} Olympics took place.
                    </p>
                  </div>
                )}
                  ...
                  >
                    ...
                    <div className="bg-gray-50 border border-gray-200 p-3.5 rounded-[8px] space-y-2 text-left">
                      ...
                      <p className="text-xs font-black text-gray-955 uppercase tracking-tight">
                        Host: {currentHost.city}, {currentHost.country} ({currentHost.year})
                      </p>
                      <p className="text-[11px] text-gray-600 leading-relaxed font-semibold font-sans mt-1 border-t border-gray-200 pt-1.5">
                        <strong className="text-gray-900 font-bold">Olympic Trivia:</strong> {currentHost.fact}
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      id="btn_next_olympic_round"
                    >
                      <span>{currentRoundIndex + 1 === OLYMPIC_HOSTS.length ? "Finish Challenge" : "Next Host City"}</span>
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 font-semibold mt-1">
              Final result log of your Olympic host city locations proximity evaluations.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 border-y border-gray-150 py-6" id="summary_stat_grid">
            <div>
              ...
              <span className="block text-[9px] text-gray-500 uppercase tracking-widest mt-1">out of {OLYMPIC_HOSTS.length * MAX_POINTS} max</span>
            </div>
            ...
            <div>
              ...
              <span className="block text-[9px] text-gray-500 uppercase tracking-widest mt-1">from exact hosts</span>
            </div>
          </div>
            ...
            <ul className="space-y-2 text-xs font-semibold text-gray-600">
              {resultsList.map((r) => (
                <li key={r.round} className="flex items-center justify-between py-1 border-b border-gray-100 last:border-0">
                  <span>
                    Round {r.round}: <strong className="text-gray-900">{r.host.year} {r.host.city}</strong>
                  </span>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-2">
            ...
            <button
              onClick={() => onFinishGame(totalScore, "olympic-cities")}
